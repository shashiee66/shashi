 ...
 *   DNP Slave functionality for Object 22 Binary Counter Change events.
 */
#ifndef SDNPO022_DEFINED
#define SDNPO022_DEFINED
...
extern "C" {
   ...
   * purpose: Add a binary counter event to event queue
   * arguments:
   ...
   *  void
   */
  TMWDEFS_SCL_API void TMWDEFS_GLOBAL sdnpo022_addEvent(
    TMWSESN *pSession, 
   ...
   * purpose: Initialize binary counter change events
   * arguments:
   *  pSession - pointer to session
   * returns:
   *  void
   ...
   * purpose: Start binary counter change event scanning if configured
   * arguments:
   *  pSession - pointer to session
   * returns:
   *  void
   ...
   * purpose: Close binary counter event processing
   * arguments:
   *  pSession - pointer to session
   * returns:
   *  void
   ...
   * purpose: Count the number of binary counter change events in
   *  queue
   * arguments:
   *  pSession - pointer to session
   ...
   * purpose: Read binary counter change events using specified variation
   * arguments:
   *  pSession - pointer to session
   ...
   * purpose: Read binary counter change events using variation 0 for a specific 
   *  class
   * arguments:
   *  pSession - pointer to session
   *  pResponse - response buffer to read into
